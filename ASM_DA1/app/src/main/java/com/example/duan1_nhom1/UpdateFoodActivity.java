...
public class UpdateFoodActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        maNH = food.getMaNH();
        foodTypeDAO = new FoodTypeDAO(UpdateFoodActivity.this);
        addSpinner();
    ...
    private void uploadFirebaseStogare() {
        ...
        if(edtName.getText().toString().isEmpty() || edtPrice.getText().toString().isEmpty() || edtDescribe.getText().toString().isEmpty()){
            Toast.makeText(UpdateFoodActivity.this, "Please enter enough information", Toast.LENGTH_SHORT).show();
            return;
        }
        Date date = new Date();
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                mountainsRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        ...
                        FoodDAO foodDAO = new FoodDAO(UpdateFoodActivity.this);
                        foodDAO.update(food);
                        myRef.child(String.valueOf(food.getMaMA())).setValue(food, new DatabaseReference.CompletionListener() {
                            @Override
                            public void onComplete(@Nullable DatabaseError error, @NonNull DatabaseReference ref) {
                                if(error == null){
                                    Toast.makeText(UpdateFoodActivity.this, "Update food succsess", Toast.LENGTH_SHORT).show();
                                    Intent intent = new Intent(getApplicationContext(), FoodManagerActivity.class);
                                    intent.putExtra("maNH", maNH);
                                    startActivity(intent);
                                    finish();
                                }else {
                                    Toast.makeText(UpdateFoodActivity.this, "Update food fail", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
                    }
